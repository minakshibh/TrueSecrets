...
@interface Secret7ViewController ()

@end

@implementation Secret7ViewController
@synthesize scrollView,couponView,enterCoupontxt,applyCoupon;
...
- (void)viewDidLoad
{
    ...
    if (UI_USER_INTERFACE_IDIOM()== UIUserInterfaceIdiomPad)
    {
        ...
        lblSecretNo.text = @"Secret #7";
        lblSecretNo.font = [UIFont fontWithName:@"IM FELL DW Pica SC" size:22];
        [scrollView addSubview:lblSecretNo];
        
        UILabel *lable = [[UILabel alloc] initWithFrame:CGRectMake(215, 103, 400, 25)];
        lable.text = @"400 people strip-searched by police";
        ...
        lable.text =@"What were the Victoria Police thinking when they conducted a raid that cost them a $4M legal settlement?";
        ...
        UILabel *mapDetails = [[UILabel alloc] initWithFrame: CGRectMake(25, 358, 740, 210)];
        ...
        mapDetails.text = @"It's 2am in the Tasty Nightclub when suddenly the music stops and cops arrive. 'Cool! strippers we thought...but then they started yelling and bashing and ordered us to strip naked.'";
        mapDetails.numberOfLines = 15;
        [scrollView addSubview:mapDetails];
        
        
        mapText1 = [[UILabel alloc] initWithFrame: CGRectMake(38, 177, 740, 45)];
        ...
        //mapText.tag=i;
        mapText1.text = @"Find 331 Flinders Lane and look down the alley-way to the door at the far left corner. Only the cool people know \nabout this entrance.";
        ...
        //[getBonusSoundButton setImage:[UIImage imageNamed:@"Get-bg.png"] forState:UIControlStateNormal];
        [thanksBtn setTitle:@"thanks" forState:UIControlStateNormal];
        thanksBtn.layer.cornerRadius=14;
        thanksBtn.clipsToBounds=YES;
        thanksBtn.frame = CGRectMake(120, 800, 540, 25);
        thanksBtn.backgroundColor = [UIColor colorWithRed:55.0f/255.0f green:48.0f/255.0f blue:50.0f/255.0f alpha:1.0f];
        [thanksBtn addTarget:self action:@selector(playThanks:) forControlEvents:UIControlEventTouchUpInside];
        [scrollView addSubview:thanksBtn];
        
        thanksPlaybtn=[UIButton buttonWithType:UIButtonTypeCustom];
        [thanksPlaybtn setImage:[UIImage imageNamed:@"play.png"] forState:UIControlStateNormal];
        thanksPlaybtn.layer.cornerRadius=5;
        thanksPlaybtn.clipsToBounds=YES;
        //    thanksPlaybtn.tag=2;
        thanksPlaybtn.frame = CGRectMake(615, 800, 20, 25);
        thanksPlaybtn.backgroundColor = [UIColor colorWithRed:55.0f/255.0f green:48.0f/255.0f blue:50.0f/255.0f alpha:1.0f];
        [thanksPlaybtn addTarget:self action:@selector(playThanks:) forControlEvents:UIControlEventTouchUpInside];
        [scrollView addSubview:thanksPlaybtn];
    ...
    lable.numberOfLines = 3;
    lable.textAlignment=NSTextAlignmentCenter;
    lable.text =@"What were the Victoria Police thinking when they \nconducted a raid that cost them a $4M legal settlement?";
    ...
    mapDetails.text = @"It's 2am in the Tasty Nightclub when suddenly the music \nstops and cops arrive. 'Cool! strippers we thought...but then they started yelling and bashing and ordered \nus to strip naked.'";
    mapDetails.numberOfLines = 15;
    [scrollView addSubview:mapDetails];
    ...
    //mapText.tag=i;
    mapText1.text = @"Find 331 Flinders Lane and look down the alley-way to the \ndoor at the far left corner. Only the cool people know \nabout this entrance.";
    [scrollView addSubview:mapText1];
    ...
    thanksBtn.layer.cornerRadius=14;
    thanksBtn.clipsToBounds=YES;
    thanksBtn.frame = CGRectMake(75, 800, 170, 25);
    thanksBtn.backgroundColor = [UIColor colorWithRed:55.0f/255.0f green:48.0f/255.0f blue:50.0f/255.0f alpha:1.0f];
    [thanksBtn addTarget:self action:@selector(playThanks:) forControlEvents:UIControlEventTouchUpInside];
    [scrollView addSubview:thanksBtn];
    ...
    thanksPlaybtn.clipsToBounds=YES;
    //    thanksPlaybtn.tag=2;
    thanksPlaybtn.frame = CGRectMake(205, 800, 20, 25);
    thanksPlaybtn.backgroundColor = [UIColor colorWithRed:55.0f/255.0f green:48.0f/255.0f blue:50.0f/255.0f alpha:1.0f];
    [thanksPlaybtn addTarget:self action:@selector(playThanks:) forControlEvents:UIControlEventTouchUpInside];
    [scrollView addSubview:thanksPlaybtn];
...
-(IBAction)handleGestureLeft:(id)sender
{
    ...
    ismasterInit=FALSE;isBonusInit=FALSE;isBackgroundInit=FALSE;isthanksInit=FALSE;
...
- (void)getbackgroundSoundbtn:(id)sender
{
    ...
    {
        [self handleSingleTapGesture];        return;
    }
    ...
    if (backgroundvalue==TRUE)
    {
        ...
        if (isBackgroundInit==TRUE)
        {
            ...
            backgroundMusicFile = @"Secret_7_Background";
...
- (void)playbutton:(id)sender
{
  ...
     playButton1=[UIButton buttonWithType:UIButtonTypeCustom];
    ...
    [scrollView addSubview:playButton1];
    musicplayfile = @"police_raid_master";
...
-(void)getBonusSoundBtn:(id)sender
{
    ...
    else
    {
        ...
        else
        {
            ...
            [getBonusBtn2 setImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
            backgroundMusicFile = @"Secret_7_Bonus";
            NSURL *url=[NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"%@",backgroundMusicFile] ofType:@"mp3"]];
...
-(void)playThanks:(id)sender
{
    ...
    {
        [thanksAudioPlayer pause];
        [thanksAudioPlayer prepareToPlay];
        [thanksPlaybtn setImage:[UIImage imageNamed:@"play.png"] forState:UIControlStateNormal];
    ...
    {
        if (isthanksInit==TRUE)
        {
            [thanksAudioPlayer play];
            [thanksPlaybtn setImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
            //playing=FALSE;
            thanksvalue=TRUE;
        ...
        else
        {
            ...
            [thanksPlaybtn setImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
            //            backgroundMusicFile = [bonusMeterial objectAtIndex:playbutton3];
            NSURL *url=[NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"Thanks-track"] ofType:@"mp3"]];
            thanksAudioPlayer
            ...
            thanksAudioPlayer=[[AVAudioPlayer alloc]initWithContentsOfURL:url error:nil];
            [thanksAudioPlayer prepareToPlay];
            thanksAudioPlayer.delegate=self;
            [thanksAudioPlayer play];
            isthanksInit=TRUE;
            thanksvalue=TRUE;
...
-(void)showMe1:(id)sender
{
    ...
    if (!bool_showmap==TRUE)
    {
        ...
        showMeImage.image=[UIImage imageNamed:@"show7.jpg"];
        /*
         break;
         case 2:
...
- (void) cordinate
{
    ...
    region.center.latitude = -37.817554;
    region.center.longitude = 144.963226;
